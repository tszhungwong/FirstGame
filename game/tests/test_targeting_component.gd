...
const TARGETING_COMPONENT_PATH := "res://combat/components/targeting_component.gd"


func test_selects_the_nearest_target_within_range() -> void:
	var targeting_script := load(TARGETING_COMPONENT_PATH) as GDScript
	assert_not_null(targeting_script)
	if targeting_script == null:
		return

	var targeting: Node = autofree(targeting_script.new())
